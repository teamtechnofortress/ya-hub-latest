...
<div class="p-sm-4 p-3 project">
    <div class="py-4">
        <form action="{{route('saveEstimate')}}" method="post" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="estimate_id" value="{{$template_data[0]->id}}">
            <div class="row">
                            ...
                            @php
                                $dept = DB::table('departments')
                                    ...
                                    $notetemps = DB::table('note_templatefor_deps')
                                                ->where('user_id', Auth::user()->id)
                                                ->where('depid',$dept->id)
                                                ->where('notefor','estimate')
                                                ->get();

                                    $notetemp = DB::table('note_templatefor_deps')
                                                ->where('id', $template[0]->notesestimateid)
                                                ->where('user_id', Auth::user()->id)
                                                ->where('notefor','estimate')
                                                ->get();
                            ...
                            {{-- <div class="col-lg-12 text-left">
                                <div class="form-group mt-4">
                                    ...
                                    <select class="custom-select" name="notestempid" required>
                                        ...
                                        <option value="{{ $item->id }}" {{ $item->id == $template[0]->notesestimateid ? 'selected' : '' }}>{{ $item->notename }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div> --}}
