...
<div class="p-sm-4 p-3 project">
    <div class="py-4">
        ...
        <div class="row mt-2 advanceFilter" style="display: none">
            <div class="col-lg-6">
                <form action="{{url('filter_sale_month')}}" method="post">
                    @csrf
            ...
            <div class="col-lg-6">
                <form action="{{url('filter_sale_year')}}" method="post">
                    @csrf
                    <div class="form-group">
        ...
        <form action="{{url('add_sales')}}" method="post" id="form-task" data-action="">
            @csrf
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group mt-4">
                        ...
                        <input type="number" step="0.01" class="form-control task_date" name="monthlysale" id="end_date" placeholder="Sale" required>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group mt-4">
                        <label for="end_date">Cost</label>
                        <input type="number" step="0.01" class="form-control task_date" name="cost" id="cost" placeholder="Cost" required>
                    </div>
                </div>
                <div class="col-md-6">
                    {{-- <div class="form-group mt-4">
                ...
                <div class="table-responsive">
                    <table class="table table-stripped table-bordered" id="sales_table" style="position: static;">
                        ...
                        <tbody> 
                            ...
                            <td>
                            <form action="{{route('changeSaleCurrency',$item->id)}}" method="POST" class="currencyForm">
                                   @csrf
                                ...
                                <td class="currency-amount">{{$currency}} {{$item->monthlysale}}</td>
                                <td>{{$currency}} {{$item->cost}}</td>
                                <!-- <td>employ of the year</td> -->
                                @php
                                $val = $item->cost/$item->monthlysale *100;
                                @endphp

                                <td>{{100 - number_format($val, 2, '.', '')}} %</td> 
                                <td>{{$currency}} {{$item->monthlysale -$item->cost}}</td>
                                <td>
                                     <a href="{{ url('updatesales/' . $item->id) }}" class="btn btn-light btn-sm">
                                        <i class="fas fa-edit"></i>
                                    </a> 
                                    <a href="{{ url('deletesale/'. $item->id) }}" class="btn btn-danger btn-sm deletesale">
   ...
   function initializeDataTableWithLogo(logoUrl) {
    ...
    $('#sales_table').DataTable( {
        ...
        buttons: [
            {
                ...
                customize: function (doc) {
                    ...
                    text: 'Export PDF',
                    exportOptions: {
                        columns: [0, 2, 3, 4, 5]  // Specify the columns you want to include in the PDF export (0-indexed)
                    }
                },
                {
                    extend: 'csvHtml5',
        ...
        order: [[0, 'asc']], // Sort by the third column (index 2), which is the Date column
            columnDefs: [
                { targets: [1, 2, 3, 4], orderable: true }, // Disable sorting for all columns except the Date column
            ]
