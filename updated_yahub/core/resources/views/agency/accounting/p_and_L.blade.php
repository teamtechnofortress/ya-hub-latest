...
<div class="p-sm-4 p-3 project">
    <div class="py-4">
        ...
        <div class="row mt-2 advanceFilter" style="display: none">
            <div class="col-lg-6">
                <form action="{{url('filter_pl_month')}}" method="post">
                    @csrf
            ...
            <div class="col-lg-6">
                <form action="{{url('filter_pl_year')}}" method="post">
                    @csrf
                    <div class="form-group">
        ...
        <div class="row mt-4">
            <div class="col-md-12 col-sm-12">
                <div class="table-responsive">
                    <table class="table table-stripped table-bordered" id="pl_table" style="position: static;">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>Money In</th>
                        ...
                        <tbody>
                        @isset($pl_month_filter) 
                            @foreach($pl_month_filter as $records) 
                            ...
                            <tr>
                                ...
                                <td class="">{{ date('Y-m-d', strtotime($detail->date))}}</td>
                                <td class="currency-amount">{{$currency}} {{$detail->total}}</td>
                                <td class="currency-amount1">{{$currency}} 0</td>
                                @endif
                                @if($detail->type === 'MO | Money Out')
                                <td class="">{{ date('Y-m-d', strtotime($detail->date))}}</td>
                                <td class="currency-amount">{{$currency}} 0</td>
                                <td class="currency-amount1">{{$currency}} {{$detail->total}}</td>
                                @endif
                                {{-- <td>
                            ...
                            @isset($pl_year_filter) 
                            @foreach($pl_year_filter as $records) 
...
<script>
    $(document).ready(function(){
        ...
        function updateTotals() {
            let totalsString = 'Total Money In&nbsp;&nbsp;'
    ...
    $(document).ready(function(){
        ...
        // build the totals string and append to the table
        function updateTotals() {
            let totalsString = 'Total Money Out&nbsp;&nbsp;'
            for (let currency in totals) {
                totalsString += currency + '' + totals[currency].toLocaleString() + `${currency!='£' ? '&nbsp;&nbsp;' : ''}`
            }
            // totalsString = totalsString.substring(0, totalsString.length - 2) // remove trailing comma and space
    ...
    $(document).ready(function(){
        ...
        let etotalsString = '&nbsp;&nbsp;';
        let ecurrencySymbol = '€'; // Set the currency symbol

        let e = totals['€'] - totals1['€'];
        etotalsString += ecurrencySymbol + '' + e.toLocaleString() + `${ecurrencySymbol !== '£' ? '&nbsp;&nbsp;' : ''}`;

        $('#tot').html(totalsString + etotalsString);
        // console.log(totals['$']);
        // console.log(totals1['$']);
    })
