    ...
    <div class="py-4">
        <div class="row">
            ...
            <div class="col-lg-6 text-right">
                ...
                <a  href="#0" data-toggle="modal" data-target="#addTaskModal" class="btn btn-primary btn-sm add">Add Filing +</a>
            </div>
        </div>
        <div class="col-md-12 task-form" style="display:none">
        <form action="{{url('add_filling')}}" method="post" id="form-task" data-action="">
            @csrf
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group mt-4">
                        ...
                        <input type="date" name="duedate" id="contact" class="form-control contact_task_id" required />
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group mt-4">
                        <label for="currency">Currency</label>
                        <select name="currency" id="currency" class="form-control task_status">
                ...
                <div class="col-md-6">
                    <div class="form-group mt-4">
                        ...
                        <input type="number" class="form-control task_date" step="0.01" name="amount" id="end_date" placeholder="Amount" required>
                    </div>
                </div>
        ...
        <div class="row mt-4">
            <div class="col-md-12 col-sm-12">
                <div class="table-responsive">
                    <table class="table table-stripped table-bordered" id="filling_table" style="position: static;">
                        <thead>
                            <tr>
                            	@php
                        ...
                        <tbody> 
                            ...
                            <tr>
                                ...
                                <td>{{ \Carbon\Carbon::parse($item->duedate)->format('Y-m-d') }}</td>
                                <td>
                                <form action="{{route('changeFillingCurrency',$item->id)}}" method="POST" class="currencyForm">
                                   @csrf
                                ...
                                <td>
                                <a href="{{ url('updatefilling/' . $item->id) }}" class="btn btn-light btn-sm">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="{{ url('deletefilling/'. $item->id) }}" class="btn btn-danger btn-sm deleteFiling">
...
<script>
    ...
    $(".deleteFiling").on("click", function(e) {
        var $this = $(this);
...
$(document).ready(function() {
    function initializeDataTableWithLogo(logoUrl) {
        ...
        if ($.fn.DataTable.isDataTable('#filling_table')) {
            $('#filling_table').DataTable().destroy();
        }
    $('#filling_table').DataTable( {
        ...
        buttons: [
            {
                ...
                customize: function (doc) {
                    ...
                    text: 'Export PDF',
                    exportOptions: {
                        columns: [0, 1, 3]  // Specify the columns you want to include in the PDF export (0-indexed)
                    }
                },
                {
                    extend: 'csvHtml5',
        ...
        order: [[1, 'asc']], // Sort by the third column (index 2), which is the Date column
            columnDefs: [
                { targets: [0, 2, 3], orderable: true }, // Disable sorting for all columns except the Date column
            ]
