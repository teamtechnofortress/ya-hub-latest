    ...
    <div class="py-4">
        <div class="row">
            ...
            <div class="col-lg-6 text-right">
                ...
                <a  href="#0" data-toggle="modal" data-target="#addTaskModal" class="btn btn-primary btn-sm add">Add Reconciliation +</a>
            </div>
        </div>
        <div class="col-md-12 task-form" style="display:none">
        <form action="{{url('add_reconciliation')}}" method="post" id="form-task" data-action="">
            @csrf
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group mt-4">
                        ...
                        <input type="text" name="department" id="dep" class="form-control task_date" placeholder="Enter department" required>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group mt-4">
        ...
        <div class="row mt-4">
            <div class="col-md-12 col-sm-12">
                <div class="table-responsive">
                    <table class="table table-stripped table-bordered" id="reconc_table" style="position: static;">
                        <thead>
                            <tr>
                            	@php
                        ...
                        <tbody> 
                            ...
                            @endphp
                            <tr>
                                {{-- <td><input type="text" name="dep" id="dep" value="sales dept."></td> --}}
                                <td>{{$item->department}}</td>
                                <td>{{ \Carbon\Carbon::parse($item->date)->format('Y-m') }}</td>
                                <td>
                                <form action="{{route('changeReconCurrency',$item->id)}}" method="POST" class="currencyForm">
                                   @csrf
                                ...
                                <td>
                                    <a href="{{ url('updatereconcilation/' . $item->id) }}" class="btn btn-light btn-sm">
                                        <i class="fas fa-edit"></i>
                                    </a> 
                                    <a href="{{ url('deletereconciliation/'. $item->id) }}" class="btn btn-danger btn-sm deleterecon">
...
<script>
    ...
    $(".deleterecon").on("click", function(e) {
        var $this = $(this);
...
$(document).ready(function() {
    function initializeDataTableWithLogo(logoUrl) {
        ...
        if ($.fn.DataTable.isDataTable('#reconc_table')) {
            $('#reconc_table').DataTable().destroy();
        }
    $('#reconc_table').DataTable({
        dom: 'lBfrtip',
        orderable:false,
        ...
        buttons: [
            ...
            },
            {
                ...
                text: 'Export CSV',
                exportOptions: {
                    columns: [0, 1, 3] // Specify the columns you want to include in the CSV export (0-indexed)
                }
            },
            'copyHtml5',
            'excelHtml5',
        ...
        order: [[1, 'asc']],
        columnDefs: [
            { targets: [0, 2, 3], orderable: false }, // Disable sorting for all columns
