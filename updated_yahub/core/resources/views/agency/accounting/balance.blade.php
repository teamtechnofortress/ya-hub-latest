...
<div class="p-sm-4 p-3 project">
    <div class="py-4">
        ...
        <div class="row mt-2 advanceFilter" style="display: none">
            <div class="col-lg-6">
                <form action="{{url('filter_balance_month')}}" method="post">
                    @csrf
            ...
            <div class="col-lg-6">
                <form action="{{url('filter_balance_year')}}" method="post">
                    @csrf
                    <div class="form-group">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group mt-4">
                        ...
                        <input type="month"  name="month" id="contact" class="form-control contact_task_id" required />
                    </div>
                </div>
                <!-- <div class="col-md-6">
                    <div class="form-group mt-4">
                        <label for="task">Balance</label>
                        <input type="number" step="0.01" class="form-control task_title" name="balance" id="task" placeholder="Enter Balance" required>
                    </div>
                </div> -->
                <div class="col-md-6">
                    <div class="form-group mt-4">
                ...
                <div class="col-md-6">
                    <div class="form-group mt-4">
                        ...
                        <textarea name="note" id="note" class="form-control task_note" placeholder="Description"></textarea>
                    </div>
                </div>
                <div class="col-md-6">
                ...
                <div class="table-responsive">
                    <table class="table table-stripped table-bordered" id="balance_table" style="position: static;">
                        ...
                        <tbody> 
                            ...
                            <tr>
                                <td>{{$item->id}}</td>
                                <td>{{ \Carbon\Carbon::parse($item->date)->format('F') }}</td>
                                <td>{{ \Carbon\Carbon::parse($item->date)->format('Y') }}</td>
                                <td>{{$currency}} {{$cur_bal}}</td>
                                <td>
                                <form action="{{route('changeBalCurrency',$item->id)}}" method="POST" class="currencyForm">
                                   @csrf
                                ...
                                <td>{{$item->type}}</td>
                                @if($item->type == 'Debit')
                                <td class="currency-amountD">{{$currency}} {{$item->amount}}</td>
                                @endif
                                @if($item->type == 'Credit')
                                <td class="currency-amountC">{{$currency}} {{$item->amount}}</td>
                                @endif
                                 
                                <!-- <td class="currency-amount1">{{$currency}} 500</td> -->
                                <td>{{$item->description}}</td>
                                <td>
                                    <a href="{{ url('updatebalance/' . $item->id) }}" class="btn btn-light btn-sm">
                                        <i class="fas fa-edit"></i>
                                    </a> 
                                    <a href="{{ url('deletebalance/'. $item->id) }}" class="btn btn-danger btn-sm deleteBal">
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('.table .currency-amountC').each(function() {
 ...
 $(document).ready(function(){
        ...
        // loop through each table cell with a currency class
        $('.table .currency-amountD').each(function() {
            let text = $(this).text(); // get the text content of the cell
            let currency = text.substring(0, 1); // get the currency symbol from the text
            let amount = parseFloat(text.substring(2)); // get the numerical amount from the text
            totals[currency] += amount; // add the amount to the correct currency's total
...
$('.add,.cancel').click(function(){
    ...
    $(document).ready(function() {
    // Function to fetch logo and initialize DataTable
    function initializeDataTableWithLogo(logoUrl) {
        ...
        // Initialize DataTable with logo
