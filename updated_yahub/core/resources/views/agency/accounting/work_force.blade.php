    ...
    <div class="py-4">
        <div class="row">
            ...
            <div class="col-lg-6 text-right">
                ...
                <a  href="#0" data-toggle="modal" data-target="#addTaskModal" class="btn btn-primary btn-sm add">Add Workforce +</a>
            </div>
        </div>
        <div class="col-md-12 task-form" style="display:none">
            <form action="{{url('add_workforce')}}" method="post" id="form-task" data-action="">
                @csrf
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group mt-4">
                            ...
                            <input type="text" name="description" id="contact" class="form-control contact_task_id" required />
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group mt-4">
                            <label for="contractor">Cost</label>
                            <input type="number" step="0.01" name="costph" id="contact" class="form-control contact_task_id" required />
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group mt-4">
                            <label for="contractor">Unit</label>
                            <input type="number" step="0.01" name="unit" id="unit" class="form-control contact_task_id" required />
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group mt-4">
                            <label for="contact">Taxes %</label>
                            <input type="number" step="0.01" name="taxes" id="contact" class="form-control contact_task_id" required />
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group mt-4">
                            <label for="contact">Total</label>
                            <input type="number" step="0.01" name="totalh" id="contact" class="form-control contact_task_id" required />
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group mt-4">
        ...
        <div class="row mt-4">
            <div class="col-md-12 col-sm-12">
                <div class="table-responsive">
                    <table class="table table-stripped table-bordered" id="work_force_table" style="position: static;">
                        <thead>
                            <tr>
                            	@php
                        ...
                        <tbody> 
                            ...
                            <tr>
                                <td>
                                <form action="{{route('changeWorkCurrency',$item->id)}}" method="POST" class="currencyForm">
                                   @csrf
                                ...
                                <td class="">{{$currency}} {{$item->costPerHouers}}</td>
                                <td>{{$item->unit}}</td>
                                <td class=""> {{$item->taxes}}</td>
                                {{-- <td><input type="number" id="tax" step="0.01" min="0" placeholder="" value="10.00"> </td> --}}
                                <td class=""> {{$item->totalHoures}}</td>
                                @php
                                $tax = $item->costPerHouers * $item->taxes / 100;
                                $tot_tax = $tax * $item->totalHoures;
                                $tot_cost = $item->costPerHouers * $item->totalHoures;

                                @endphp
                                <td class="currency-amount">{{$currency}} {{$tot_cost + $tot_tax}}</td>
                                {{-- <td><input type="text" name="desc" id="desc"></td> --}}
                                
                                <td class=""> {{$item->description}}</td>
                                <td>
                                     <a href="{{ url('updateworkforce/' . $item->id) }}" class="btn btn-light btn-sm">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="{{ url('deleteworkforce/'. $item->id) }}" class="btn btn-danger btn-sm deleteWork">
...
$(document).ready(function() {
    function initializeDataTableWithLogo(logoUrl) {
        ...
        if ($.fn.DataTable.isDataTable('#work_force_table')) {
            $('#work_force_table').DataTable().destroy();
        }
    $('#work_force_table').DataTable( {
        dom: 'lBfrtip',
        ...
        buttons: [
            {
                ...
                customize: function (doc) {
                    ...
                    text: 'Export PDF',
                    exportOptions: {
                        columns: [1, 2, 3, 4, 5]  // Specify the columns you want to include in the PDF export (0-indexed)
                    }
                },
                {
                    extend: 'csvHtml5',
