...
<div class="p-sm-4 p-3 project">
    <div class="py-4">
        ...
        <form action="{{url('add_expenses')}}" method="post" id="form-task" data-action="">
            @csrf
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group mt-4">
                        ...
                        <input type="text" name="expense" id="contact" class="form-control contact_task_id" required /> 
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group mt-4">
                    ...
                    <table class="table table-stripped table-bordered " style="position: static;" id="expense_table">
                        ...
                        <tbody> 
                            ...
                            <tr>
                                <td>{{$item->expense}}</td>
                                <td>
                                <form action="{{route('changeExpenseCurrency',$item->id)}}" method="POST" class="currencyForm">
                                   @csrf
                                ...
                                <td>
                                     <a href="{{ url('updateexpenses/' . $item->id) }}" class="btn btn-light btn-sm">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="{{ url('deleteexpenses/'. $item->id) }}" class="btn btn-danger btn-sm deleteExpense">
...
$('.add,.cancel').click(function(){
    ...
    $('#expense_table').DataTable( {
        ...
        buttons: [
            {
                ...
                customize: function (doc) {
                    ...
                    text: 'Export PDF',
                    exportOptions: {
                        columns: [0, 2, 3]  // Specify the columns you want to include in the PDF export (0-indexed)
                    }
                },
                {
                    extend: 'csvHtml5',
        ...
        order: [[3, 'asc']], // Sort by the third column (index 2), which is the Date column
            columnDefs: [
                { targets: [0, 1, 2], orderable: true }, // Disable sorting for all columns except the Date column
            ]
        });
    }
