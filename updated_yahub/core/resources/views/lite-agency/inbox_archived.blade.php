@extends('layouts.lite-agency') @section('content') <div class="border-bottom m-list px-sm-4 px-3 py-3 sticky-top bg-white">
    ...
    <div class="d-flex justify-content-between align-items-center w-100 py-2 ml-3 pr-2">
        <a href="{{url('chat/'.$chat->id)}}"
            ...
            <div>
                <h2>@if(!empty($message)) @if($message->sender_id!=\App\Models\User::where('id',\Auth::user()->id)->first()->id && $message->is_delivered==0)<strong> {{!empty($chat->client)? $chat->client->name:""}} | {{!empty($chat->project) ? $chat->project->project_title:""}} </strong> @else {{!empty($chat->client)? $chat->client->name:""}} | {{!empty($chat->project) ? $chat->project->project_title:""}} @endif @else {{!empty($chat->client)? $chat->client->name:""}} |
                    {{!empty($chat->project) ? $chat->project->project_title:""}} @endif </h2>
                <p class="wrap-text">@if(!empty($message)) @if($message->sender_id!=\App\Models\User::where('id',\Auth::user()->id)->first()->id && $message->is_delivered==0) <strong>{{$message->message}}</strong> @else {{$message->message}} @endif @endif </p>
            </div>
        </a>
    ...
    $('.archive').click(function() {
        ...
        axios.post("{{route('lite-unarchive-post')}}", formdata).then(res => {
