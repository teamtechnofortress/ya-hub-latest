...
class ChatController extends Controller
{
    ...
    public function chat_ajax(Request $request)
    {
        ...
        if($action=="send_message")
        {
            ...
            $message=$request->input("message");
            $messageCount = Message::where(['sender_id' => $sender_id, 'chat_id' => $chat_id])->count();
            $project=User::where("id",$sender_id)->first();
            $message_limit = $project->max_conversations_in_inbox;
            if ($messageCount >= $message_limit) {
                return json_encode(["success" => false, "error" => "You can only send up to {$message_limit} messages."], JSON_PRETTY_PRINT);
            }
        ...
        if($action=="send_image")
        {
            ...
            $project=User::where("id",$sender_id)->first();
            $maxFileSizeInMB  = $project->value;
            // if ($file->getSize() > $maxFileSizeInMB * 1024 * 1024) {
            //     return json_encode(["success" => false, "error" => "You can only send up to {$maxFileSizeInMB} messages."], JSON_PRETTY_PRINT);
            // }
