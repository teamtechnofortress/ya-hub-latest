...
class Admin extends Controller
{
    ...
    public function liteagencyInvoiceSetting($id,$value){
        $user=User::where('id',$id)->first();
        $user->enable_invoiceTool=(int)$value;
        if($user->update())
        {
            return redirect()->back()->withSuccess('Invoice Module Changed!');
        }
        return redirect()->back()->withErrors('Something went wrong!');
    }
    ...
    public function userInvoiceSetting($id,$value){
        $user = User::where('id',$id)->first();
        $user->enable_invoiceTool = (int)$value;
        if($user->update())
        {
    ...
    public function agencyInvoiceSetting($userId,$val){
        $user = User::find($userId);
        $user->enable_invoiceTool = (int)$val;
        if($user->save())
        {
            return redirect()->back()->withSuccess('Invoice Module Updated!');
        }
        return redirect()->back()->withErrors('error','Invoice Module Cannot Update!');
    }

    public function bookkeepingTool($userId,$val){
        $user = User::find($userId);
        $user->bookkeepingTool = (int)$val;
        if($user->save())
        {
            return redirect()->back()->withSuccess('Bookkeeping Module Updated!');
        }
        return redirect()->back()->withErrors('error','Bookkeeping Module Cannot Update!');
    }

    public function accountingTool($userId,$val){
        $user = User::find($userId);
        $user->accountingTool = (int)$val;
        if($user->save())
        {
            return redirect()->back()->withSuccess('Accounting Module Updated!');
        }
        return redirect()->back()->withErrors('error','Accounting Module Cannot Update!');
    }

    public function departments_module($userId,$val){
        $user = User::find($userId);
        $user->departments = (int)$val;
        if($user->save())
        {
            return redirect()->back()->withSuccess('Departments Module Updated!');
        }
        return redirect()->back()->withErrors('error','Accounting Module Cannot Update!');
    }
    
    private function wrapLinksInAnchorTags($text)
    {
